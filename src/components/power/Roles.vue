<template>
    <div>
        ...
        <el-dialog title="提示" :visible.sync="setRightDialogVisible" width="50%" @close="setRightDialogClosed">
            <!-- 树型控件 -->
            <el-tree ref="treeRef" default-expand-all node-key="id" :data="rightsList" :props="treeProps"  show-checkbox :default-checked-keys="defKeys"></el-tree>
...
export default {
  ...
  methods: {
    ...
    // 根据id删除对应的权限
    async removeRightById (role, rightld) {
      // 弹框提示是否确认删除
      ...
      const { data: res } = await this.$http.delete(`roles/${role}/rights/${rightld}`)
      if (res.meta.status !== 200) {
